...
### 2026-04-01 · 日记图片系统升级（上传 + 渲染 + 压缩）

本次更新为日记系统补齐“图文能力”，实现从上传 → 存储 → 渲染的完整链路。

#### 🖼 图片上传能力
- 新增后台图片上传接口：`POST /admin/notes/upload`
- 支持在新建 / 编辑日记时上传图片
- 上传后返回可直接用于 Markdown 的路径
- 支持一键插入或复制路径到正文
...
#### 📝 Markdown 图片支持
- 日记详情页已支持标准 Markdown 渲染
- 可直接使用：

```md
![描述](/uploads/notes/xxx.jpg)
...
所有图片处理逻辑集中在服务端上传接口
前台仅负责渲染，不参与处理
保持数据结构不变（无 schema 变更）
优先保证简单、稳定、可控
⚠️ 注意事项
当前图片存储在项目本地目录（public/uploads）
本地开发与 VPS 部署需分别同步该目录
暂不适用于无状态部署平台（如 Vercel）
