...
export default async function EditBroadcastPage({
  ...
  const updateBroadcastWithId = updateBroadcast.bind(null, numericId);
      ...
      <div className="max-w-xl space-y-6">
        ...
        <nav className="text-sm text-stone-500">
          <div className="flex items-center gap-2">
            ...
            <span>/</span>
            <span className="text-stone-700">编辑广播 #{row.id}</span>
          </div>
        </nav>

        {/* 表单卡片 */}
        <div className="rounded-md border border-stone-200 bg-white px-6 py-6">
          <h1 className="text-xl font-semibold tracking-tight text-stone-800">编辑广播</h1>
          <p className="mt-2 text-sm text-stone-500">
            修改后保存，内容立即同步到前台。
          </p>

          <form action={updateBroadcastWithId} className="mt-6 space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full rounded-sm border border-stone-200 bg-stone-50 px-3 py-2 text-sm text-stone-800 outline-none transition focus:border-emerald-400 focus:bg-white focus:ring-1 focus:ring-emerald-200"
              />
              <p className="text-xs text-stone-400">图片路径（如 /images/avatar.png）或单个汉字</p>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full rounded-sm border border-stone-200 bg-stone-50 px-3 py-2 text-sm text-stone-800 outline-none transition focus:border-emerald-400 focus:bg-white focus:ring-1 focus:ring-emerald-200"
              />
              <p className="text-xs text-stone-400">仅用于前台展示，格式随意</p>
            </div>
