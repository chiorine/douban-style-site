...
export default async function EditNotePage({
  ...
  const row = await prisma.note.findUnique({ where: { id: numericId } });
  if (!row) notFound();

  // 将 JSON 存储字段还原为可编辑字符串
  const tagsDefaultValue = (JSON.parse(row.tags) as string[]).join(", ");
  const contentDefaultValue = (JSON.parse(row.content) as string[]).join(
    "\n\n",
  );
  const updateNoteWithId = updateNote.bind(null, numericId);
      ...
      <div className="max-w-xl space-y-6">
        ...
        <nav className="text-sm text-stone-500">
          <div className="flex items-center gap-2">
            ...
            <span>/</span>
            <span className="text-stone-700">编辑日记 #{row.id}</span>
          </div>
        </nav>

        {/* 表单卡片 */}
        <div className="rounded-md border border-stone-200 bg-white px-6 py-6">
          <h1 className="text-xl font-semibold tracking-tight text-stone-800">编辑日记</h1>
          <p className="mt-2 text-sm text-stone-500">
            修改后保存，内容立即同步到前台。
          </p>

          <form action={updateNoteWithId} className="mt-6 space-y-5">
